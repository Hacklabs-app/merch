...
export default function EditCategoryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchCategory = async () => {
    try {
      ...
      const category = data.categories?.find((c: any) => c.id === parseInt(resolvedParams.id as string));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      await adminUpdateCategory(parseInt(resolvedParams.id as string), formData);
  ...
  const handleDelete = async () => {
    try {
      await adminDeleteCategory(parseInt(resolvedParams.id as string));
