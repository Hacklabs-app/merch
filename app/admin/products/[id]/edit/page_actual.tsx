...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchProduct = async () => {
    try {
      ...
      const product = data.products?.find((p: any) => p.id === parseInt(resolvedParams.id as string));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      await adminUpdateProduct(parseInt(resolvedParams.id as string), formData);
  ...
  const handleDelete = async () => {
    try {
      await adminDeleteProduct(parseInt(resolvedParams.id as string));
